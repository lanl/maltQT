...
public:
  ...
  std::map<std::string,struct instrMapItem> &instrMap() { return instrMap_;} //< instrMap accessor
  std::map<std::string,std::vector<std::string>> &nameMap() {return nameMap_;} //< nameMap accessor
...
private:
  ...
  std::map<std::string,struct instrMapItem> instrMap_; //< Lookup for converting instruction to file / function / line
  std::map<std::string,std::vector<std::string>> nameMap_; //< Reverse lookup from function to stacks
};
